...
sync_core.py
------------
Trendyol + Hepsiburada senkronizasyonunun tüm paylaşılan config sabitlerini
ve iş mantığını (route OLMAYAN her şeyi) barındırır: kimlik bilgisi/host
sabitleri, HTTP istemci fonksiyonları (trendyol_get/hepsiburada_get),
Hepsiburada paket/finans fetch+dönüştürme mantığı, incremental senkron
cursor yardımcıları, arka plan senkron orkestrasyonu (_run_full_sync/
_run_hb_sync/_run_sync_in_thread) ve dashboard/aylık kâr özetleyicileri.

app.py sadece Flask app kurulumu + auth + blueprint kaydı içerir;
blueprints/dashboard_routes.py, blueprints/order_routes.py ve
blueprints/finance_routes.py buradan import eder. Bu dosyanın kodu app.py'den
BİREBİR taşındı (davranış değişmedi) — bkz. her bloğun üstündeki
"kaynak: app.py satır ..." notu.
"""
...
# ---- kaynak: app.py satır 74-97 (Trendyol config sabitleri) ----
...
# ---- kaynak: app.py satır 161-197 (_run_sync_in_thread) ----
...
# ---- kaynak: app.py satır 216-260 (Hepsiburada config sabitleri) ----
...
# ---- kaynak: app.py satır 265-271 (_check_hb_credentials) ----
def _check_hb_credentials():
    ...
    return None
# ---- kaynak: app.py satır 274-293 (hepsiburada_get / hepsiburada_finance_get) ----
...
# ---- kaynak: app.py satır 296-352 (fetch_all_hb_packages) ----
...
# ---- kaynak: app.py satır 453-473 (fetch_all_hb_shipped/delivered_packages) ----
...
# ---- kaynak: app.py satır 476-536 (_hb_money / _hb_iso_to_epoch_ms / _hb_line_rows_for) ----
def _hb_money(value):
...
# ---- kaynak: app.py satır 539-632 (sync_hb_packages_to_db) ----
...
# ---- kaynak: app.py satır 635-707 (HB settlement-only backfill) ----
...
# ---- kaynak: app.py satır 738-1052 (HB finans senkronu) ----
...
def _json_dump_safe(obj):
    ...
    try:
        ...
        return None
# ---- kaynak: app.py satır 1055-1110 (Incremental senkronizasyon yardımcıları) ----
...
# ---- kaynak: app.py satır 1112-1166 (_run_hb_sync) ----
...
# ---- kaynak: app.py satır 1205-1218 (_check_credentials (Trendyol)) ----
# ============================================================
...
# ---- kaynak: app.py satır 1220-1238 (trendyol_get) ----
...
# ---- kaynak: app.py satır 1241-1271 (fetch_all_orders) ----
def fetch_all_orders(start_ts_ms, end_ts_ms, status=None):
    ...
    return all_orders
# ---- kaynak: app.py satır 1274-1282 (_date_chunks) ----
def _date_chunks(start_dt, end_dt, max_days=14):
    ...
    while cur < end_dt:
        ...
        cur = chunk_end
    return chunks
# ---- kaynak: app.py satır 1285-1326 (get_daily_returns) ----
...
# ---- kaynak: app.py satır 1329-1375 (sync_orders_to_db) ----
...
# ---- kaynak: app.py satır 1378-1430 (_run_full_sync) ----
...
# ---- kaynak: app.py satır 1433-1449 (_resolve_sync_range) ----
...
# ---- kaynak: app.py satır 1452-1496 (_dashboard_summary_for_marketplace / _monthly_profit_for_marketplace) ----
